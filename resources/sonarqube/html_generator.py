...
def generate_severity_plot(df):
    ...
    colors = {
        'CRITICAL': '#d62728',  # Muted red
        'MAJOR': '#ff7f0e',    # Muted orange
        'MINOR': '#9467bd'     # Muted purple
    }
    bar_colors = [colors.get(severity, color) for severity, color in zip(severity_counts.index, sns.cubehelix_palette(len(severity_counts), start=2, rot=0, reverse=True))]
...
def generate_file_plot(df):
    ...
    plt.figure(figsize=(20, 12))
    bars = plt.barh(file_counts.index, file_counts.values, color=sns.cubehelix_palette(10, start=2, rot=0, reverse=True))
...
def generate_category_plot(df):
    ...
    plt.figure(figsize=(20, 12))
    category_counts.plot.pie(autopct="%.1f%%", colors=sns.cubehelix_palette(len(category_counts), start=2, rot=0, reverse=True), startangle=90, fontsize=22, textprops={'fontsize': 20})
...
def generate_issue_type_plot(df):
    ...
    plt.figure(figsize=(20, 12))
    issue_type_counts.plot.pie(autopct="%.1f%%", colors=sns.cubehelix_palette(len(issue_type_counts), start=2, rot=0, reverse=True), startangle=90, fontsize=22, textprops={'fontsize': 20})
...
def generate_vulnerability_prob_plot(df):
    ...
    plt.figure(figsize=(20, 12))
    bars = plt.bar(vulnerability_prob_counts.index, vulnerability_prob_counts.values, color=sns.cubehelix_palette(len(vulnerability_prob_counts), start=2, rot=0, reverse=True))
...
def generate_hotspot_file_plot(df):
    ...
    plt.figure(figsize=(20, 12))
    bars = plt.barh(hotspot_file_counts.index, hotspot_file_counts.values, color=sns.cubehelix_palette(10, start=2, rot=0, reverse=True))
